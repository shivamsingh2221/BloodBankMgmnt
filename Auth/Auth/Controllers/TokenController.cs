    ...
    public class TokenController : ControllerBase
    {
        ...
        private string GenerateJSONWebToken(Authenticate userInfo)
        {
            ...
            List<Claim> claims = new List<Claim>() {
                new Claim(JwtRegisteredClaimNames.Sub, userInfo.Usern),
                new Claim(ClaimTypes.NameIdentifier, userInfo.Usern.ToString()),
                new Claim(JwtRegisteredClaimNames.Jti, Guid.NewGuid().ToString())
            };
            if (userInfo.Usern == "admin")
            {
        ...
        private Authenticate AuthenticateUser(Authenticate login)
        {
            ...
            if (login.Usern.Equals("admin") && login.Pass.Equals("admin"))
            {
